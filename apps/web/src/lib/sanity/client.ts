 ...
 * silently shipping the project id and dataset to the browser. That is why the
 * env vars below have no NEXT_PUBLIC_ prefix.
 */
export const client = createClient({
  ...
  // Data Cache holds the responses and the revalidate endpoint drops them by
  // tag, so a second cache in between would only delay invalidation.
  useCdn: false,
